...
def CreateVecTable(Base, TableSize=None):
    Vectors = [
        ('InitialSP',),                                                                                                 # 0000
        ('Reset',),                                                                                                     # 0004 (-3)
...
def CreateSegmentSRAM():
    startEA = _RAM_BASEADDR
    endEA = _RAM_BASEADDR + _RAM_SIZE - 1

    seg = ida_segment.getseg(startEA)
    Name = ida_segment.get_segm_name(seg) if seg else None
    if Name == _RAM_NAME and startEA == seg.startEA and endEA == seg.endEA:
        # SRAM already exist
        pass
    else:
        ...
        idaapi.add_segm_ex(seg, _RAM_NAME, 'CODE', 0)
...
def nonfunction_first_instruction_heuristic(EAstart, EAend):
    ea = EAstart
    ...
    while ea < EAend:
        ...
        if function_name != "":
            ...
            if (mnem == "PUSH" or mnem == "PUSH.W" or mnem == "STM" or mnem=="MOV"):
...
def load_file(li, neflags, format):
   ...
   CreateVecTable(base_address, 0xE8)
   CreateEnums()
   ...
   nonfunction_first_instruction_heuristic(base_address, _FLASH_BASEADDR + _FLASH_SIZE)
